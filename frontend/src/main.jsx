 ...
 return <section className="desktop-workspace-panel apps-workspace apps-workspace-v3">
   <div className="apps-main-pane">
     <header className="workspace-header apps-main-header">
       <div><h1>{selected?selected.title:'Приложения и списки'}</h1></div>
       {selected&&<button className="workspace-back" onClick={()=>setSelectedId(null)}>К спискам</button>}
     </header>
     {selected?<div className="compact-list-stage" onPointerDown={e=>{if(e.target===e.currentTarget&&window.matchMedia('(min-width:901px) and (hover:hover) and (pointer:fine)').matches)setSelectedId(null)}}><ChecklistCard checklist={selected} focused onBack={()=>setSelectedId(null)}/></div>:
     <div className="apps-content-scroll">
       <section className="apps-section">
         <div className="apps-section-head"><h2>Приложения</h2><button className="apps-inline-add" onClick={()=>onEditApp(addSlot)}>＋ Добавить</button></div>
         <div className="workspace-tile-grid">
           {apps.map(app=><button className="workspace-tile app-workspace-tile" key={`${app._slot}-${app.url}`} onClick={()=>onOpenApp({title:app.title||app.name||'Приложение',url:app.url})}>
             <span className="workspace-tile-icon app-icon">{(app.title||app.name||'A')[0].toUpperCase()}</span>
             <span className="workspace-tile-copy"><b>{app.title||app.name}</b><small>{String(app.url||'').replace(/^https?:\/\//,'').replace(/\/$/,'')}</small></span>
             <em className="workspace-tile-menu" onClick={e=>{e.stopPropagation();onEditApp(app._slot)}}>⋮</em>
           </button>)}
           {!apps.length&&<div className="apps-empty-note">Приложений пока нет</div>}
         </div>
       </section>
       <hr className="apps-divider"/>
       <section className="apps-section lists-section">
         <div className="apps-section-head"><h2>Совместные списки</h2><button className="apps-inline-add" onClick={openNewList}>＋ Добавить</button></div>
         {busy?<div className="apps-empty-note">Загрузка…</div>:!lists.length?<div className="apps-empty-note">Создайте первый совместный список</div>:
         <div className="workspace-tile-grid">
   ...
   <aside className="apps-action-column">
     <button className="apps-action-button" onClick={()=>onEditApp(addSlot)}><span>＋</span><b>Добавить приложение</b></button>
     <button className="apps-action-button" onClick={openNewList}><span>＋</span><b>Добавить список</b></button>
   </aside>
   {showCreate&&<StandaloneChecklistCreateModal close={()=>setShowCreate(false)} created={row=>{setShowCreate(false);setLists(xs=>[row,...xs.filter(x=>x.id!==row.id)]);setSelectedId(row.id)}}/>}
 </section>
}
...
const CONTACT_CATEGORY_ORDER=['family','friends','work','other','groups'];
const CONTACT_CATEGORY_LABELS={family:'Семья',friends:'Друзья',work:'Работа',other:'Другие',groups:'Группы'};
